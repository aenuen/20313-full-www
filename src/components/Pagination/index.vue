  ...
  >
    <div class="layui-box layui-laypage layui-laypage-default">
      <a
          ...
          @click.prevent="home()"
      >
        <i v-if="showType === 'icon'" class="layui-icon layui-icon-prev" />
        <template v-else>首页</template>
      </a>
      <a :class="{'layui-disabled': current === 0}" @click.prevent="prev()">
        <i v-if="showType === 'icon'" class="layui-icon layui-icon-left" />
      ...
      <template v-for="(item,index) in pages">
        <a
            ...
            href="javascript:" :key="'page' + index"
      ...
      <a :class="{'layui-disabled': current === pages.length - 1}" @click.prevent="next()">
        <i v-if="showType === 'icon'" class="layui-icon layui-icon-right" />
      ...
      <a
          ...
          @click.prevent="end()"
      >
        <i v-if="showType === 'icon'" class="layui-icon layui-icon-next" />
        <template v-else>尾页</template>
      </a>
    </div>
    <div class="total" v-if="hasTotal">
      到第
      <input type="text" class="goto-input text-center" @keyup.enter="jumpTo" />
      页 共 {{ totalPages }} 页
    </div>
    <div v-if="hasSelect">
      <div
...
export default {
  props: {
    ...
    showType: { type: String, default: 'icon' },
    showEnd: { type: Boolean, default: false },
    theme: { type: String, default: 'layui-bg-green' },
