  ...
  <transition name="fade">
    <div class="layui-layer-page layui-layer-border edit-content" v-show="isShow">
        ...
        <ul class="layui-form layui-form-pane">
          <li class="layui-form-item">
            ...
            <button type="button" class="layui-btn layui-btn-primary">
              <label for="uploadImg">
                <i class="layui-icon layui-icon-upload" />上传图片
              </label>
            </button>
            <input
                type="file"
      ...
      <span class="layui-layer-setwin" @click.stop="close()">
        <a href="javascript:" class="layui-layer-ico layui-layer-close layui-layer-close1" />
      </span>
    </div>
  </transition>
</template>
...
export default {
  name: 'ComponentsEditorImgUpload',
  props: ['isShow', 'ctrl'],
  data () {
    return {
      pic: '',
  ...
  methods: {
    ...
    upload (e) {
      ...
      // 上传图片
      contentUpload(this.formData).then((res) => {
        if (res.code === 200) {
          this.pic = res.data
        }
      })
